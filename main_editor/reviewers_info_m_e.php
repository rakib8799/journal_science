        ...
        <div class="col-lg-12 col-12">
            <div class="card mt-2 shadow p-3 mb-5 bg-body rounded">
                <div class="card-header">
                    <h3 class = "text-center text-secondary fw-bold">Reviewers Information</h3>
                </div>
                <div class="card-body">
                            ...
                            <tbody>
                                <?php
                                    if(isset($_POST['search']))
                                    {
                                        ...
                                        if($name!=NULL)
                                        {
                                            $select_ass_editor_info = "SELECT * FROM `reviewer_information` WHERE `reviewer_name` like '%$name%'";
                                        }
                                        else
                                        {
                                            ?>
                                            <script>
                                                ...
                                                window.location = "reviewers_info_m_e.php";
                                            </script>
                                            <?php
                                        }
                                    ...
                                    {
                                        $select_ass_editor_info = "SELECT * FROM `reviewer_information` ORDER BY `id` DESC";
                                    }
                                    ...
                                    while($row = mysqli_fetch_assoc($run_select_ass_editor_info))
                                    {
                                        ?>
                                            <tr>
                                                ...
                                                <td><?php echo $row['reviewer_university_name']?></td>
                                                <td><?php echo $row['reviewer_designation']?></td>
                                                <td><?php echo $row['reviewer_email']?></td>
                                                <td><?php echo $row['reviewer_country']?></td>
                                                <td><?php echo $row['reviewer_contact_no']?></td>
                                                
                                                <form action="" method = "POST">
                                                <?php 
                                                    if($row['reviewer_status']==0)
                                                    {
                                                        ?>
                                                            <td class = "text-danger fw-bold">Inactive</td>
                                                            <td>
                                                                <input type="hidden" name = "id" value = "<?php echo $row['id'] ?>">
                                                                <input type="hidden" name = "email" value = "<?php echo $row['reviewer_email'] ?>">
...
<?php 
    if(isset($_POST['active']))
    {
        $id = $_POST['id'];
        $email = $_POST['email'];
        $update_ass_editor_info = "UPDATE `reviewer_information` SET `reviewer_status` = '1' WHERE `id` = '$id'";
        ...
        if($run_update_ass_editor_info)
        {
            ...
            $body = '<h5>Dear Sir/Madam, <br /> Your request to register as a reviewer has been accepted. You can now login to the site.<br /> <br /> Best Regards, JKKNIU Journal Organization</h5>';
            $send_mail = send_mail($receiver, $subject, $body);
        }
        ?>
            <script>
                window.alert("A Mail Has Been Sent To The Reviewer");
                window.location = "reviewers_info_m_e.php";
            </script>
        <?php 
    }
    else if(isset($_POST['inactive']))
    {
        $id = $_POST['id'];
        $email = $_POST['email'];
        $update_ass_editor_info = "UPDATE `reviewer_information` SET `reviewer_status` = '0' WHERE `id` = '$id'";
        $run_update_ass_editor_info = mysqli_query($conn, $update_ass_editor_info);
        if($run_update_ass_editor_info)
        {
            $receiver = $email;
            $subject = "Banned as an Reviewer";
            $body = '<h5>Dear Sir/Madam, <br /> You have been suspended to work as a reviewer.<br /> <br /> Best Regards, JKKNIU Journal Organization</h5>';
            $send_mail = send_mail($receiver, $subject, $body);
        }
        ?>
            <script>
