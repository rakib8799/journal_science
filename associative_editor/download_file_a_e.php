<?php
    ...
    $count_error = 0;
    if(isset($_GET['type']) && isset($_GET['file']) && isset($_GET['paper_id']) && isset($_GET['associative_editor_id']))
    {
        if($_GET['type']=='manuscript' || $_GET['type'] == 'cover_letter' || $_GET['type']=='supplimentary_file')
        {
            ...
            {
                $paper_id_validation = "SELECT * FROM `new_paper` WHERE `paper_id` = '$_GET[paper_id]'  AND `$column_name` = '$_GET[file]' AND `paper_status`='1' AND `associative_editor_id` = ''";
            }
            else if($_GET['associative_editor_id']==1)
            {
                $paper_id_validation = "SELECT * FROM `new_paper` WHERE `paper_id` = '$_GET[paper_id]'  AND `$column_name` = '$_GET[file]' AND `associative_editor_id` = '$_SESSION[associative_editor_id]'";
            }
            else 
            {
                $count_error++;
            ...
            if($num_paper_id>0 && $count_error==0)
            {   
